...
function _jamul_find_delimiter($string, $pos=0, $min = -1, $max=-1) {
	...
	if ($pos == 0 || $pos < 0 || $pos >= $len) { return $pos; }

	if ($min < 0) { $min = max(0, $pos - _JAMUL_LEN_SEARCH); }

	if ($max < 0 || $max >= $len) { $max = min($len - 1, $pos + _JAMUL_LEN_SEARCH); }
	...
	if (ord($string[$pos]) < 0x80) {
		...
		if ($pos2 > $max) {
			...
			if ($pos2 < $min && $pos3 >= 0) { $pos2 = $pos3; }
		}

		if ($pos2 <= $max && $pos2 >= $min) { $pos = $pos2; }
